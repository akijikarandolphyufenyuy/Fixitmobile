...
class _PaymentScreenState extends State<PaymentScreen>
  ...
  static const _kApiUrl        = 'https://mesomb.hachther.com/api/v1.1/payment/collect/';
  ...
  static String _buildAuthorization({
    ...
    final canonicalHeaders = sortedKeys.map((k) => '$k:${headers[k]}').join('\n');
    ...
    final d     = date;
    final scope = '${d.year}${d.month.toString().padLeft(2,'0')}${d.day.toString().padLeft(2,'0')}/payment/mesomb_request';
    final stringToSign = 'HMAC-SHA1\n$timestamp\n$scope\n${sha1.convert(utf8.encode(canonicalRequest)).toString()}';
    final signature    = Hmac(sha1, utf8.encode(_kSecretKey)).convert(utf8.encode(stringToSign)).toString();
  ...
  Future<void> _pay() async {
    ...
    setState(() { _processing = true; _failed = false; _failMessage = ''; });
    ...
    try {
      ...
      final body  = <String, dynamic>{
        ...
        'currency': 'XAF',
        'fees'    : true,
        'message' : widget.purpose == 'post_job' ? 'Fixit - Job Posting Fee' : 'Fixit - Unlock Job Details',
      ...
      final response = await http.post(
        Uri.parse(_kApiUrl),
        headers: {
          'Authorization'       : _buildAuthorization(date: date, nonce: nonce, body: body),
          'Content-Type'        : 'application/json; charset=utf-8',
      ...
      if (response.statusCode >= 400) {
        throw Exception(data['detail'] ?? data['message'] ?? 'HTTP ${response.statusCode}');
      }
      ...
      if (uid.isNotEmpty) {
        await TransactionRepository().saveTransaction(AppTransaction(
          userId       : uid,
          ...
          paidAt       : DateTime.now(),
          status       : 'success',
          mesombRef    : (data['transaction'] as Map?)?.cast<String,dynamic>()['pk'] as String?,
        ));
      }
