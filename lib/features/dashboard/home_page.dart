...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(minutes: 1), (_) {
      setState(() => _now = DateTime.now().toUtc().add(const Duration(hours: 1)));
    });
  ...
  Future<void> _loadJobs() async {
    ...
    if (!mounted) return;
    setState(() { _recentJobs = jobs.take(5).toList(); _loadingJobs = false; });
  }

  Future<void> _loadMyJobs() async {
    setState(() => _loadingMyJobs = true);
    final uid = _authRepo.currentUser?.id;
    if (uid == null) { setState(() => _loadingMyJobs = false); return; }
    final jobs = await _jobsRepo.getJobsByPostedBy(uid);
    if (!mounted) return;
    setState(() { _myJobs = jobs.take(3).toList(); _loadingMyJobs = false; });
  }

  Future<void> _loadApps() async {
    setState(() => _loadingApps = true);
    final uid = _authRepo.currentUser?.id;
    if (uid == null) { setState(() => _loadingApps = false); return; }
    final jobs = await _jobsRepo.getJobsByPostedBy(uid);
    int count = 0;
    ...
    if (!mounted) return;
    setState(() { _pendingApps = count; _loadingApps = false; });
  }
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          slivers: [
              ...
              flexibleSpace: FadeTransition(
                ...
                child: StreamBuilder<List<AppNotification>>(
                  ...
                  builder: (context, snap) {
                    ...
                    return _HomeHeader(
                      displayName: GoRouterState.of(context).extra as String? ?? '',
                      greeting: _greeting,
            ...
            SliverToBoxAdapter(child: _buildSectionTitle('Recent Jobs', '/dashboard/jobs', isDark)),
            SliverToBoxAdapter(child: _buildRecentJobs(isDark)),
            SliverToBoxAdapter(child: _buildSectionTitle('My Posted Jobs', '/dashboard/my-jobs', isDark)),
            SliverToBoxAdapter(child: _buildMyJobs(isDark)),
            SliverToBoxAdapter(child: _buildSectionTitle('Applications', '/dashboard/applications', isDark)),
  ...
  Widget _buildSectionTitle(String title, String route, bool isDark) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(title, style: TextStyle(color: isDark ? const Color(0xFFF5EDE5) : _kBrown, fontSize: 17, fontWeight: FontWeight.w700)),
          GestureDetector(
            onTap: () => context.go(route),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
              decoration: BoxDecoration(color: _kOrange.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(20)),
              child: const Text('See all', style: TextStyle(color: _kOrange, fontSize: 12, fontWeight: FontWeight.w700)),
            ),
          ),
        ],
      ),
  ...
  Widget _buildRecentJobs(bool isDark) {
    ...
    if (_recentJobs.isEmpty) {
      return _EmptyState(message: 'No jobs available yet', icon: Icons.work_off_rounded, isDark: isDark);
    }
    return SizedBox(
      height: 180,
      child: ListView.builder(
        ...
        itemBuilder: (context, i) => _JobHorizontalCard(job: _recentJobs[i], index: i, isDark: isDark),
      ),
    );
  }
  ...
  Widget _buildMyJobs(bool isDark) {
    ...
    return Column(
      children: _myJobs.asMap().entries.map((e) => _MyJobCard(job: e.value, index: e.key, isDark: isDark)).toList(),
    );
  }
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.description_rounded, color: _kOrange, size: 26),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text('Pending Applications', style: TextStyle(color: isDark ? const Color(0xFFF5EDE5) : _kBrown, fontSize: 15, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 4),
                    Text(
                      _loadingApps ? 'Loading...' : '$_pendingApps applicants waiting for review',
                      style: const TextStyle(color: _kBrownMid, fontSize: 13),
                    ),
                  ],
                ),
              ),
              const Icon(Icons.chevron_right_rounded, color: _kOrange, size: 24),
            ],
          ),
        ),
      ),
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    widget.label,
                    style: TextStyle(
                      color: widget.labelColor.withValues(alpha: 0.65),
                      fontSize: 12,
...
class _JobHorizontalCard extends StatelessWidget {
  ...
  const _JobHorizontalCard({required this.job, required this.index, required this.isDark});
          ...
          child: Column(
            ...
            children: [
              ClipRRect(
                ...
                child: Image.asset(_asset(job.category), height: 90, width: double.infinity, fit: BoxFit.cover),
              ),
              Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(job.title, style: TextStyle(color: isDark ? const Color(0xFFF5EDE5) : _kBrown, fontSize: 13, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
                    const SizedBox(height: 4),
                    Row(
                      children: [
                        const Icon(Icons.location_on_rounded, size: 12, color: _kBrownMid),
                        const SizedBox(width: 3),
                        Expanded(child: Text(job.location, style: const TextStyle(color: _kBrownMid, fontSize: 11), overflow: TextOverflow.ellipsis)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                      decoration: BoxDecoration(color: _kOrange.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)),
                      child: Text(job.category, style: const TextStyle(color: _kOrange, fontSize: 10, fontWeight: FontWeight.w700)),
                    ),
                  ],
                ),
              ),
...
class _MyJobCard extends StatelessWidget {
  ...
  const _MyJobCard({required this.job, required this.index, required this.isDark});
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ClipRRect(
                  borderRadius: BorderRadius.circular(10),
                  child: Image.asset(_asset(job.category), width: 56, height: 56, fit: BoxFit.cover),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(job.title, style: TextStyle(color: isDark ? const Color(0xFFF5EDE5) : _kBrown, fontSize: 14, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 3),
                      Row(
                        children: [
                          const Icon(Icons.location_on_rounded, size: 12, color: _kBrownMid),
                          const SizedBox(width: 3),
                          Text(job.location, style: const TextStyle(color: _kBrownMid, fontSize: 12)),
                        ],
                      ),
                    ],
                  ),
                ...
                Container(
                  ...
                  child: const Text('View', style: TextStyle(color: _kOrange, fontSize: 12, fontWeight: FontWeight.w700)),
                ),
              ],
            ),
          ),
        ...
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.add_circle_outline_rounded, color: _kOrange, size: 26),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Post Your First Job', style: TextStyle(color: isDark ? const Color(0xFFF5EDE5) : _kBrown, fontSize: 15, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 3),
                  const Text('Tap to post a job and find skilled workers', style: TextStyle(color: _kBrownMid, fontSize: 12)),
                ],
              ),
            ),
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(message, style: const TextStyle(color: _kBrownMid, fontSize: 14)),
          ],
        ),
      ),
    );
...
class _HomeHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final firstName = displayName.isNotEmpty ? displayName.split(' ').first : 'there';
    return Container(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(greeting, style: TextStyle(color: Colors.white.withValues(alpha: 0.80), fontSize: 12, fontWeight: FontWeight.w400)),
                  const SizedBox(height: 2),
                  Text('Hey, $firstName 👋',
                      style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700, letterSpacing: -0.3, height: 1.1),
